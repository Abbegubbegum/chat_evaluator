...
def export():
    ...
    test_results_sheet.append(["ID", "Test", "Test Name", "Message", "Result"])
    ...
    for i, test_result in enumerate(test_results_table.all()):
        ...
        test_results_sheet.append([test_result.doc_id, test_result['test'], f"=VLOOKUP(B{row};{tests_sheet_name}!$A$2:$B${len(tests_table) + 1};2)",
                                  test_result['message'], test_result['result']])
    ...
    for i, message in enumerate(messages_table.all()):
        ...
        messages_sheet.append([message.doc_id, message['sender'], f"=VLOOKUP(B{row};{models_sheet_name}!$A$2:$B${len(
            models_table) + 1};2)", message['game'], f"=VLOOKUP(D{row};{games_sheet_name}!$A$2:$E${len(games_table) + 1};3)", message['content'], message['turn'], f"=COUNTIFS('{test_results_sheet_name}'!$D$2:$D${len(test_results_table) + 1};A{row};'{test_results_sheet_name}'!$E$2:$E${len(test_results_table) + 1};TRUE)", f"=COUNTIFS('{test_results_sheet_name}'!$D$2:$D${len(test_results_table) + 1};A{row};'{test_results_sheet_name}'!$E$2:$E${len(test_results_table) + 1};FALSE)", f"=H{row}/(H{row}+I{row})"])
